...
export GATYGO_SYSROOT="$FIXTURES/sysroot" GATYGO_STATE="$tmp/state" GATYGO_ASSETS="$tmp/assets" \
       GATYGO_RUN="$tmp/run" GATYGO_LOG="$tmp/gatygo.log" GATYGO_INIT=/src/tests/stubs/gatygo-init \
...
# the daemon logs to stderr; collect it in GATYGO_LOG so the leak check below sees every line

# --- no URL configured
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "1" "$?" "no sub_url -> error"
...
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "0" "$?" "first update succeeds"
...
assert_eq "running reload" "$(tr '\n' ' ' < "$GATYGO_INIT_LOG" | sed 's/ $//')" "init asked: running? then reload"
assert_exit 0 "xray.json installed" test -s "$GATYGO_STATE/xray.json"
assert_eq "600" "$(stat -c %a "$GATYGO_STATE/xray.json")" "xray.json is 0600"
assert_eq "600" "$(stat -c %a "$GATYGO_STATE/subscription.json")" "subscription.json is 0600"
...
: > "$GATYGO_INIT_LOG"
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "0" "$?" "unchanged update succeeds"
...
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "0" "$?" "fallback is not an error"
assert_eq "warning" "$(_last RESULT)" "RESULT=warning"
assert_eq "🌐 Auto" "$(_last PROFILE)" "fell back to the first balancer profile"
uci set gatygo.main.profile="📍 Bravo"; gatygo_update 2>>"$GATYGO_LOG"
...
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "1" "$?" "device limit -> error"
...
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "1" "$?" "HTTP 500 -> error"
...
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "1" "$?" "unreachable -> error"
uci set gatygo.main.sub_url=http://127.0.0.1:8789/sub
uci set gatygo.main.user_agent="curl/8.0"
gatygo_update 2>>"$GATYGO_LOG"; assert_eq "1" "$?" "wrong format (unknown UA) -> error"
...
assert_eq "0" "$(grep -c '127.0.0.1' "$GATYGO_LOG")" "log never contains the subscription host"
...
GATYGO_INIT_RUNNING_RC=1 gatygo_update 2>>"$GATYGO_LOG"
